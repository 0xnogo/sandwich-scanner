...
describe('Fetcher', () => {
  ...
  it('should get transaction details', async () => {
    // GIVEN
    providerWrapperMockInstance.callProviderWithRetriesAndWait = jest
      .fn()
    ...
    expect(
      providerWrapperMockInstance.callProviderWithRetriesAndWait,
    ).toHaveBeenCalledTimes(2);
  });

  it('should get transaction details by block number and index', async () => {
    // GIVEN
    providerWrapperMockInstance.callProviderWithRetries = jest
      .fn()
    ...
    // WHEN
    const result = await fetcher.getTransactionDetailsByBlockNumberAndIndex(
      1,
      1,
    );
    ...
    expect(
      providerWrapperMockInstance.callProviderWithRetries,
  ...
  it('should get transaction details from address', async () => {
    ...
    axios.get = jest.fn().mockResolvedValueOnce({
      data: { data: { items: items, pagination: { has_more: false } } },
    });
    ...
    // WHEN
    const result = await fetcher.getTransactionDetailsFromAddress(
      'DUMMY_ADDRESS',
    );
